...
L,Sigma,K,C,InvC,Xs,Ys,LB_best,elapsed_time = sGP.Train(L0,Sigma0,X,Y,N,M,NoCandidates)
...
for n in range(N_Star):
    xStar = X_Star[n]
    Y_StarMean[n], Y_StarStd[n] = sGP.Predict(Xs,xStar,L,Sigma,Ys,K,C,InvC,M)
...
plt.plot(Xs,Ys,'o',markeredgecolor='black',markerfacecolor='red',markeredgewidth=1.5,markersize=10,label='Sparse dataset')
plt.xlabel('x')
plt.legend()
plt.show()
