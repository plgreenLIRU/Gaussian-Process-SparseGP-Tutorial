...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\section{Code Speed-up}
%Our code certainly isn't optimised, but it seems like the construction of the kernel matrices provides one of the greatest opportunities for speed-up. Many GP kernels are a function of the squared distance between two input points ($\x_i$ and $\x_j$, for example). Here we describe a computationally efficient way of finding these squared distances (and therefore finding $\boldsymbol{K}_{mm}$, $\boldsymbol{K}_{nm}$, $\boldsymbol{K}_{mn}$ etc.) \\

%First, we look at the case where we have univariate inputs. We define the following row vectors:

%\begin{equation}
%	\X = (x_1,...,x_N), \qquad \X_m = (x_{m1},...,x_{mM})
%\end{equation}
%To efficiently calculate the matrix of squared distances needed to form $\boldsymbol{K}_{nm}$ (for example), we create the matrix $\tilde{\X}$ as well as the vectors $\X^{\text{\text{sq}}}$ and $\X_m^{\text{sq}}$ where

%\begin{equation}
%	\X^{\text{sq}} = (x_1^2,...,x_N^2), \qquad \X_m^{\text{sq}} = (x_{m1}^2,...,x_{mM}^2), \qquad   \tilde{\X}(i,j) = \X^{\text{sq}}(i) + \X_m^{\text{sq}}(j)
%\end{equation}
%It is then easy to show that the matrix of squared distances needed to form $\boldsymbol{K}_{nm}$ is given by

%\begin{equation}
%	\boldsymbol{D}^{\text{sq}} = -2\X^T \X_m + \tilde{\X}
%\end{equation}
%(as $\boldsymbol{D}^{\text{sq}}(i,j) = -2x_i x_{mj} + x_i^2 + x_{mj}^2 = (x_i - x_j)^2$). \\

%For the case where we have multivariate inputs, we define
...
%\begin{equation}
%	\X^{\text{sq}} = (||\x_1||^2, ..., ||\x_N||^2), \qquad \X_m^{\text{sq}} =  (||\x_{m1}||^2, ..., ||\x_{mM}||^2)
%\end{equation}
%and $\tilde{\X}$ where

%\begin{equation}
%	\tilde{\X}(i,j) = \X^{\text{sq}}(i) + \X_m^{\text{sq}}(j)
%\end{equation}
%As with the univariate case, our matrix of squared distances will then be

%\begin{equation}
%	\boldsymbol{D}^{\text{sq}} = -2\X^T\X_m + \tilde{\X}
%\end{equation}
%(as $\boldsymbol{D}^{\text{sq}}(i,j) = -2(\x_i^T\x_{mj}) + ||\x_i||^2 + ||\x_{mj}||^2 = (\x_i - \x_{mj})^T(\x_i - \x_{mj})$).
